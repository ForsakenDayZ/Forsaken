...
while {true} do 
{
	...
	if (!(vehicle player == player) && driver vehicle player == player) then {
		...
		if (BTC_Hud_Cond) then {
			BTC_liftHudId = _chopper addAction [("<t color=""#ED2744"">" + ("Lift Hud Off") + "</t>"),"\z\addons\dayz_code\external\=BTC=_Logistic\=BTC=_Lift\=BTC=_Hud.sqf", "", 0, false, false];
		} else {
			BTC_liftHudId = _chopper addAction [("<t color=""#ED2744"">" + ("Lift Hud On") + "</t>"),"\z\addons\dayz_code\external\=BTC=_Logistic\=BTC=_Lift\=BTC=_Hud.sqf", "", 0, false, false];
		};
	};
			...
			if (((abs _cargo_z) < BTC_lift_max_h) && ((abs _cargo_z) > BTC_lift_min_h) && ((abs _cargo_x) < BTC_lift_radius) && ((abs _cargo_y) < BTC_lift_radius)) then {
				if (BTC_lifted == 1) then {
					...
					if ((count (crew _cargo)) != 0) then {
						execVM "\z\addons\dayz_code\external\=BTC=_Logistic\=BTC=_Lift\=BTC=_detachCargo.sqf";
					};
				};
				if (BTC_lifted == 0 && BTC_lift == 1  && !(_cargo getVariable ["BTC_Cannot_Lift", false])) then	{
					_name_cargo  = getText (configFile >> "cfgVehicles" >> typeof _cargo >> "displayName");
					_text_action = ("<t color=""#ED2744"">" + "Lift " + (_name_cargo) + "</t>");
					BTC_liftActionId = _chopper addAction [_text_action, "\z\addons\dayz_code\external\=BTC=_Logistic\=BTC=_Lift\=BTC=_attachCargo.sqf", [_cargo, _name_cargo], 7, true, true];
					BTC_lift = 0;
				};
			};
