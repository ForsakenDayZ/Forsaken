...
TraderDialogLoadItemList = {
	...
	_item_list = [];
	{
		...
		_File = "\z\addons\dayz_code\external\gold\" + _afile + ".sqf";
...
TraderDialogBuy = {
	...
	if( 0 > 1 )then{ // _item select 9 == "\z\addons\dayz_code\external\gold\trade_items.sqf"
	
			
	
	_data = [_item select 0, _item select 1, _item select 2, _item select 8];
	
	_data execVM "\z\addons\dayz_code\external\gold\trade_items_buy.sqf";
...
TraderDialogSell = {
	...
	_data execVM "\z\addons\dayz_code\external\gold\trade_items_sell.sqf";
