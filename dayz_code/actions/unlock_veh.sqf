private["_vehicle","_key","_towed"];
_vehicle = (_this select 3) select 0; 
_key = (_this select 3) select 1;
if((_vehicle getVariable ["Towed", false])) exitWith {
		cutText [format["Cannot unlock vehicle while its being towed or lifted."], "PLAIN DOWN"];};
