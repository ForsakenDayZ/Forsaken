...
if (_inVehicle && (_vehicle isKindOf "MV22")) then {
   ...
   if (mv22_fold < 0) then {
     ...
     if !(isEngineOn themv22) then {
       mv22_fold = themv22 addAction ["Fold","z\addons\dayz_code\external\animate\mv22_fold.sqf","",5,false,true];
       mv22_unfold = themv22 addAction ["UnFold","z\addons\dayz_code\external\animate\mv22_unfold.sqf","",5,false,true];
       mv22_open = themv22 addAction ["Open Ramp","z\addons\dayz_code\external\animate\mv22_open.sqf","",5,false,true];
       mv22_close = themv22 addAction ["Close Ramp","z\addons\dayz_code\external\animate\mv22_close.sqf","",5,false,true];
     };
   };
   if (isEngineOn themv22) then {
...
if (_inVehicle && (_vehicle isKindOf "ArmoredSUV_Base_PMC")) then {
   ...
   if (suv_close < 0) then {
     thesuv = _vehicle;
     suv_close = thesuv addAction ["Close Hatch","z\addons\dayz_code\external\animate\suv_close.sqf","",5,false,true];
     suv_open = thesuv addAction ["Open Hatch","z\addons\dayz_code\external\animate\suv_open.sqf","",5,false,true];
   };
} else {
...
if (!isNull cursorTarget && !_inVehicle && !_isPZombie && (player distance cursorTarget < _allowedDistance) && _canDo) then {	//Has some kind of target
	...
	if (_player_studybody) then {
		if (s_player_studybody < 0) then {
		 // --------ZUPA - Check Wallet --------
				s_player_studybody = player addAction [("<t color=""#FF0000"">"+("Check Wallet") + "</t>"), "\z\addons\dayz_code\external\gold\check_wallet.sqf",_cursorTarget, 0, false, true, "",""];
			// ---- ZUPA END ---
		};
	} else {
	...
	if(_typeOfCursorTarget in DZE_UnLockedStorage and (player distance _cursorTarget < 3)) then {
		if (s_bank_dialog < 0) then {
				s_bank_dialog = player addAction ["Online Banking", "\z\addons\dayz_code\external\gold\bank_dialog.sqf",_cursorTarget, 3, true, true, "", ""];	
		};
	} else {
	...
	if(_typeOfCursorTarget in DZE_ATM  and (player distance _cursorTarget < 3)) then {		
		if (s_bank_dialog2 < 0) then {
			s_bank_dialog2 = player addAction ["Bank ATM", "\z\addons\dayz_code\external\gold\bank_dialog.sqf",_cursorTarget, 3, true, true, "", ""];
		};			
	} else {		
	...
	if (_isMan and _isAlive and !_isZombie and !_isAnimal and !(_traderType in serverTraders)) then {
		if (s_givemoney_dialog < 0) then {
			s_givemoney_dialog = player addAction [format["Give Money to %1", (name _cursorTarget)], "\z\addons\dayz_code\external\gold\give_player_dialog.sqf",_cursorTarget, 3, true, true, "", ""];
		};
	} else {
	...
	*/
	...
	call compile preprocessFileLineNumbers '\z\addons\dayz_code\external\mf-tow\init.sqf';
...
s_player_deploybike2 = player addaction[("<t color=""#4eff00"">" + ("Re-Pack Bike") +"</t>"),"z\addons\dayz_code\external\Bike\pack.sqf","",5,false,true,"", ""];
};
} else {
