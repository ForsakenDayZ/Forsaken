...
_randomizedLoot			= 12;				// Random number of loot piles as well as the guaranteed ones
_spawnFire				= true;				// Spawn Smoke/Fire at the helicrash
_fadeFire				= false;			// Fade the Smoke/Fire overtime
_preWaypoints			= 1;				// Amount of way points the heli flies to before crashing
_crashDamage			= 1;				// damage the heli can take before crashing (between 0.1 and 1) Lower the number and the heli can take less damage before crashing 1 damage is fully destroyed and 0.1 something like a DMR could one shot the heli
_exploRange				= 200;				// How far away from the predefined crash point should the heli start crashing
_minLootRadius			= 7;				// Minimum distance for loot to spawn from the crash site in meters
_maxLootRadius			= 12;				// Maximum distance for loot to spawn from the crash site in meters
_lootTable				= "HeliCrash";		// Name of the loot table the heli gets loot from (DO NOT edit unless you know what your doing)
_markerRadius			= 500;				// Radius for the marker
...
if (_spawnRoll <= _spawnChance) then
{
	...
	_helipilot 			= _aigroup createUnit ["US_Pilot_Light_EP1",getPos _crashwreck,[],0,"FORM"];
	_helipilot 			setCombatMode "BLUE";
	_helipilot 			moveindriver _crashwreck;
	_helipilot 			assignAsDriver _crashwreck;
	_cash = round(random 1000) * 100; // number between 0 and 20 000
	_helipilot setVariable["CashMoney",_cash ,true];
	sleep 0.5;
